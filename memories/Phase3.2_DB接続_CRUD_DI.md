...
## 🤖 Androidエンジニア向け：サーバーサイド特有の概念

Phase 3.2に入る前に、モバイル開発とは異なるサーバーサイド特有の概念を理解しておきましょう。

### 1. トランザクション（transaction）とは？

**Androidでの類似概念**: Roomの`@Transaction`アノテーション

サーバーサイドでは、複数のDB操作を「まとめて成功/失敗」させる仕組みが重要です。

```kotlin
// サーバーサイド（Exposed）
transaction {
    // この中のすべての操作が成功するか、
    // どれか1つでも失敗したら全部なかったことになる
...
### 2. コネクションプール（Connection Pool）とは？

**Androidでの類似概念**: OkHttpのConnectionPool
...
├───────────────────────────────────────────────────────────────┤
│  ユーザーA → [接続1] → DB                                      │
│  ユーザーB → [接続2] → DB     ← 同時に複数の接続が必要！        │
│  ユーザーC → [接続3] → DB                                      │
│                                                                 │
│  毎回接続を作る = 遅い（約100ms）                              │
│  → 事前に接続を作っておき、使い回す = コネクションプール        │
└───────────────────────────────────────────────────────────────┘
```

---

### 3. DTO（Data Transfer Object）とEntity の分離

**Androidでの類似概念**: UIモデルとドメインモデルの分離

サーバーサイドでは、データの「入れ物」を用途によって使い分けます：

```
┌─────────────────┐     ┌─────────────────┐     ┌─────────────────┐
│   Request DTO    │     │   Domain Entity  │     │   Response DTO   │
│  (クライアント    │  →  │   (サーバー内部   │  →  │  (クライアント    │
│   からの入力)     │     │   で処理用)       │     │   への出力)       │
└─────────────────┘     └─────────────────┘     └─────────────────┘
   @Serializable           data class             @Serializable
   CreateReportRequest     Report                 ReportDto

   ・JSONから変換          ・UUID型               ・JSONへ変換
   ・バリデーション前      ・LocalDate型          ・String型（ISO形式）
   ・idなし                ・完全な情報           ・機密情報は除外
```

**なぜ分けるのか？**
- **セキュリティ**: パスワードなど返してはいけない情報を除外
- **型安全性**: 内部ではUUID/LocalDateを使い、ミスを防ぐ
- **柔軟性**: クライアントに返す形式を自由に変更可能

---

### 4. HTTPステータスコードの使い分け

**Androidでの経験**: Retrofitで受け取るResponseのコード

サーバーを作る側になると、**適切なコードを返す責任**があります：

```
┌──────────┬──────────────────────────────────────────────────────┐
│ コード    │ 意味と使い分け                                        │
├──────────┼──────────────────────────────────────────────────────┤
│ 200 OK   │ 成功（GETでデータ返す時）                             │
│ 201 Created │ 作成成功（POSTでリソース作成後）                   │
│ 204 No Content │ 成功だが返す内容なし（DELETE成功後）           │
